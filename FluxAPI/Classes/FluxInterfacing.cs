...
namespace FluxAPI.Classes
{
    public static class FluxInterfacing
    {
        ...
        [DllImport("C:\\ProgramData\\Flux\\FluxteamAPI.dll", CallingConvention = CallingConvention.StdCall)]
        ...
        public static void inject()
        {
            switch (inject_custom())
            {
                case Result.DLLNotFound:
                    MessageBox.Show("Dynamic-Link Library (DLL) not found.", FluxFiles.Executor, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    break;
                case Result.OpenProcFail:
                    MessageBox.Show("OpenProcess failed.", FluxFiles.Executor, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    break;
                case Result.AllocFail:
                    MessageBox.Show("Allocation failed", FluxFiles.Executor, MessageBoxButtons.OK, MessageBoxIcon.Error);

                    break;
                case Result.LoadLibFail:
                    MessageBox.Show("LoadLibrary failed.", FluxFiles.Executor, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    break;
                case Result.ProcNotOpen:
                    MessageBox.Show("Couldn't find process. Make sure you have the game from the Microsoft Store.", FluxFiles.Executor, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    break;
                case Result.Unknown:
                    MessageBox.Show("An unkown error has occurred.", FluxFiles.Executor, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    break;
            }
        }
        ...
        public static void create_files(string dll_path_)
        {
            ...
            if (flag)
            {
                MessageBox.Show($"Failure when initializing {FluxFiles.Executor}\nDLL path was invalid\n",
